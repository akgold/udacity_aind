...
class MinimaxPlayer(IsolationPlayer):
    ...
    def minimax(self, game, depth):
        ...
        **********************************************************************
        ...
        # If no legal moves left, return
        if moves == []:
            return (-1, -1)

        # Define sub_fn to get values    
        def _get_val(game, max_flag, depth):
            if self.time_left() < self.TIMER_THRESHOLD:
                raise SearchTimeout()
            ...
            for new_game in new_games:
                val = max(val, max_flag * _get_val(new_game, -1 * max_flag, depth - 1))
            
            return max_flag * val

        # Get values of moves using minimax
        max_val = -float('inf')
        best_move = (-1, -1)
        for move in moves:
            val = _get_val(game.forecast_move(move), -1, depth - 1)
            if val > max_val:
                max_val = val
                best_move = move
    ...
    """Return value of move of minimizing agent.
        ...
        depth : int
            Depth is an integer representing the maximum number of plies to
            search in the game tree before abortinghether the value is to be the max 
            or the min. 1 means max, -1 means min

        max_depth:
            max_depth is an integer representing the maximum number of plies to
            search in the game tree before aborting

        Returns
        -------
        val
            the value of the move of the agent
        """
